...
net.smartworks.Swgraph = (function(){
	...
	var drawRectAnimate = function(context , rectX , rectY , rectWidth , rectHeight){

		//context.save();

		var tempGraphX = rectX;

		var iId = setInterval(function(){

			tempGraphX += 5;
			if (tempGraphX > rectWidth) {
				clearInterval(iId);
			} else {
				context.fillRect(rectX , rectY , tempGraphX , rectHeight);
			}
		},1);
	...
	var drawGraph = function(graphItemList, canvas , context) {
		...
		for (var i = 0; i < graphItemListLength; i++) {
			...
			//drawRectAnimate(context , graphX , graphY , graphItemWidth , graphItemHeight);
			// context.fill();
			// context.strokeStyle = graphItem.backgroundColor;
			// drawRoundRect(context , graphX , graphY , graphItemWidth , graphItemHeight);
			...
			if (graphItem.subCount) {
				...
				//drawRectAnimate(context, graphX , graphY + 5 , subGraphWidth , graphItemHeight -10);
				context.fill();
			}
		...
		for (var i = 0; i < graphItemListLength; i++) {
			var graphItem = graphItemList[i];
			if (graphItem.backgroundPattern) {
				fillDiagonal(canvas , context , graphItem.canvasInfo.graphX , graphItem.canvasInfo.graphY , graphItem.canvasInfo.graphItemWidth, graphItem.canvasInfo.graphItemHeight);
			}
		}
		...
		for (var i = 0; i < graphItemListLength; i++) {
			var graphItem = graphItemList[i];
			drawTitle(canvas , context , graphItem);
		}
